...
def preprocess_for_modeling(df):
    ...
    if df[config.TARGET_COLUMN].skew() > config.SKEW_THRESHOLD:
        print(f"Target variable skewness ({df[config.TARGET_COLUMN].skew():.2f}) > threshold ({config.SKEW_THRESHOLD}). Applying log transformation.")
        df[config.TARGET_COLUMN] = np.log1p(df[config.TARGET_COLUMN])
        log_transformed = True
    else:
        print("Target variable is not highly skewed. Log transformation not applied.")
    ...
    print("Data preprocessing complete. Using StandardScaler.")
    return train_df, test_df, train_rf, test_rf, scaler, log_transformed